...
const AUTH_CRAWL_BLOCKED_CLICK_LABELS =
  /\b(sign in|sign-in|log in|login|create account|show password|hide password|forgot password|forgot your password|register|sign up|sign-up|signup|reset password|continue with google|continue with apple)\b/i;
...
export function isAuthCrawlBlockedClick(label: string): boolean {
  const text = label.replace(/\s+/g, " ").trim();
  if (!text) return false;
  return AUTH_CRAWL_BLOCKED_CLICK_LABELS.test(text);
}

export function isSafeNavigationClick(label: string, options?: { authenticatedCrawl?: boolean }): boolean {
  const text = label.replace(/\s+/g, " ").trim();
  if (!text || text.length > 100) return false;
  if (BLOCKED_CLICK_LABELS.test(text)) return false;
  if (options?.authenticatedCrawl && isAuthCrawlBlockedClick(text)) return false;
...
export function isSafeBillingFlowClick(label: string, options?: { authenticatedCrawl?: boolean }): boolean {
  const text = label.replace(/\s+/g, " ").trim();
